...
plt<-ggplot(monmethall,aes(x=year,y=reltrips,
			    stratum=monmethname,
...
taf.png("output/monitoring_method_overview_trips.png")
print(plt)
dev.off()

#clean 
plt<-ggplot(monmethctry,aes(x=year,y=reltrips,
			    stratum=monmethname,
			    alluvium=monmethname,
			    fill=monmethname,
			    label=monmethname))+
	...
	labs(fill="Monitoring method")+
	ylab("Proportion of annual trips by monitoring method (%)")
taf.png("output/monitoring_method_country_trips.png")
print(plt)
dev.off()
...
monprogctry<-D2%>%group_by(monprog,ctryname,year)%>%
	...
	mutate(reltrips=trips/tot*100)
#a graph
plt<-ggplot(monprogctry,aes(x=year,y=reltrips,
			    stratum=monprog,
			    alluvium=monprog,
			    fill=monprog,
			    label=monprog))+
...
for(i in listecoregion){
	tmp<-monmethgear[monmethgear$ecoregion==i,]%>%
		filter(!is.na(L3name))
	plt<-ggplot(tmp,aes(x=year,y=trips,
			    stratum=monmethname,
			    alluvium=monmethname,
			    fill=monmethname,
			    label=monmethname))+
