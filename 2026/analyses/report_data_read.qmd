...
- what is the vocab references for the country code (ISO_3166 as a guess) into
  the WGBYC database? -> need to fully describe the format both for submission
  and for use. 


## Some numbers
Quick counting things.

```{r explo1,cache=TRUE,eval=T,echo=T,include=T}

rez1 <- D1 %>%
    filter(year > 2017) %>%
    group_by(year, ecoregion) %>%
    summarise(eff = sum(daysAtSeaF, na.rm = T))
```
...
for(i in 1:nrow(subtable)){
    for(j in 2:ncol(subtable)){
        fulltable[i,j]<-paste(subtable[i,j],paste0("(",greentable[i,j],")"),sep="\n")
    }
}
#cosmetic
...
charcolormat[(currentyear+1)==subtable[,-1]]<-"yellow"
...
#save as docx object 
save_as_docx(ftgreentable,path=here("outputs","greentable.docx"),
...
# Monitoring methods overview DAS
...
#all
monmethall<-D2%>%group_by(monitoringMethod_name,year)%>%
...
#by ctry
monmethctry<-D2%>%group_by(monitoringMethod_name,country_name,year)%>%
	summarise(das=sum(daysAtSeaOb,na.rm=T),.groups="drop") %>%
...
print(plt)
png(here("outputs","monitoring_method_overview.png"),res=300,width=1600,height=1200,unit="px")
print(plt)
dev.off()
...
plt<-ggplot(monmethctry,aes(x=year,y=reldas,
	...
	ylab("Proportion of annual DAS by monitoring method (%)")
print(plt)
png(here("outputs","monitoring_method_country.png"),res=300,width=1600,height=1200,unit="px")
print(plt)
dev.off()

#moniroting by program 
monitoringProgramType_namectry<-D2%>%group_by(monitoringProgramType_name,country_name,year)%>%
	summarise(das=sum(daysAtSeaOb,na.rm=T),.groups="drop") %>%
	group_by(year,country_name)%>%
	mutate(tot=sum(das))%>%
	ungroup()%>%
	mutate(reldas=das/tot*100)
#a graph
plt<-ggplot(monitoringProgramType_namectry,aes(x=year,y=reldas,
...
print(plt)
png(here("outputs","monitoring_program_country.png"),res=300,width=1600,height=1200,unit="px")
print(plt)
dev.off()
...
incprog<-pipo%>%group_by(monitoringMethod_name,year,newclass)%>%
...
print(plt)
png(here("outputs","incidents_by_monitoringProgram.png"),res=300,width=1600,height=1200,unit="px")
print(plt)
dev.off()
...
monmethgear<-D2%>%group_by(monitoringMethod_name,ecoregion,metierL3_name,year)%>%
	summarise(das=sum(daysAtSeaOb,na.rm=T),.groups="drop") 

#monitoring methods: ecoregion/gear view
monmethgear1<-D2%>%group_by(monitoringMethod_name,ecoregion,metierL3_name,year)%>%
	summarise(das=sum(daysAtSeaOb,na.rm=T),.groups="drop")%>%
...
monmethecoregion<-D2%>%group_by(monitoringMethod_name,ecoregion,year)%>%
	summarise(das=sum(daysAtSeaOb,na.rm=T),.groups="drop") %>%
...
#moniroting by program 
monprogctry<-D2%>%group_by(monitoringProgramType_name,country_name,year)%>%
	summarise(trips=sum(tripsOb,na.rm=T),.groups="drop") %>%
	group_by(year,country_name)%>%
	mutate(tot=sum(trips))%>%
	ungroup()%>%
...
for(i in listecoregion){
	...
	ylab("Sum of annual trips by monitoring method")
        print(plt)
	filename<-here("outputs",paste0("monitoring_method_",gsub(" ","",i),"_trips.png"))
        png(filename,res=300,width=1600,height=1200,unit="px")
	print(plt)
	dev.off()
}
...
saveWorkbook(wb,file=here("outputs","TOR_A_long_table_bycatch_only.xlsx"),overwrite=T)
...
#save as docx object 
save_as_docx(pipo,path=here("outputs","TOR_A_shorter_table.docx"),
	     pr_section=prop_section(
				page_size=page_size(orient="landscape"),
...
addWorksheet(wb,1)
writeData(wb,1,tab123)
saveWorkbook(wb,file=here("output","TOR_A_cool_effort_shorter_table.xlsx"),overwrite=T)
