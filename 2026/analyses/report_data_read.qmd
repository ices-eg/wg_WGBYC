...
author: Laurent Dubroca, Guillaume Chero, Germain Boussarie, Jani, Sam Cox, and members of TOR A WGBYC 2026 
...
in case if, for example, some data are submitted again, but this never happens almost sometimes regularly).
...
if (file.exists(here("data", "D1_wgbyc.csv"))) {
    ...
    linkD4<-paste0("https://bycatch.ices.dk/api/GetOverviewSubmissionTable/",currentyear+1)
...
Basic info for the text


```{r textmonitoringoverview,cache=TRUE,eval=T,echo=T,include=T}
nbcountryD1<-D1%>%filter(year==2025)%>%select(country_name)%>%distinct()%>%nrow()
nbcountryD2<-D1%>%filter(year==2025)%>%select(country_name)%>%distinct()%>%nrow()
nbcountryD3<-D1%>%filter(year==2025)%>%select(country_name)%>%distinct()%>%nrow()
#compute 
...
tmp<-D4%>%
    ...
    mutate(transmissionStatus=ifelse(diffdeadline>=0,"Before the deadline","After the deadline"))%>%
    mutate(transmissionStatus=ifelse(diffdeadline<0 & diffworkshop >=0,"After the deadline and before the WG",transmissionStatus))%>%
    mutate(transmissionStatus=ifelse(diffworkshop <0,"After the deadline and during the WG",transmissionStatus))
...
# ICES area and GFCM (source ICES)
...
```{r effort_monitoring_plot_map,cache=TRUE,eval=Sys.info()["sysname"]!="Darwin",echo=T,include=T}
...
list_table <- list(
  ...
  plt_monitoring_effort = c("Figure 1.4", ""), 
  plt_monitoring_coverage = c("Figure 1.5", ""), 
  data_toPlot_allPETS_plot = c("Figure 1.9", ""),
  plt_monitoring_method = c("Figure 1.6", ""), 
  plt_monitoring_program_country = c("Figure 1.7", ""),
  plt_incident_programm = c("Figure 1.8", ""), 
  plt_monitoring_method_country = c("Figure 1.10", ""),
  plt_em_total_das_trip = c("Figure 1.11", ""),
  plt_em_plt_incidents = c("Figure 1.12", "")
...
create_plot <- function(
 ...
 #   geom_sf(data = ICESareas_laea, fill = "white", color = "lightgray") +
    geom_sf(
      aes_string(
        geometry = "geometry", fill = fill_var),
      color = "darkgrey"
            ) +
   # geom_sf(data = world, fill = "lightgray", color = "darkgrey", size = 0.1) +
    scale_fill_manual(
...
textcaption <- "Metier level 3 fishing effort for 2025 (days at sea) submitted to the WGBYC database for monitoring methods retained for use under ToR C for the calculation of bycatch estimates for 2025 This includes all scientific observer data, all electronic mon-itoring data, and all vessel crew data (except for Estonia) but excludes reports from logbooks and port observers. Transparent = no data or zero."
print(textcaption)
filename <- here("outputs", glue(list_table$plt_fishing_effort[1], "_fishingEffort_L3_test.png"))
png(filename, res = 300, width = 30, height = 20, unit = "cm")
print(plt_fishing_effort)
dev.off()
read_docx() |>
...
```{r descriptive_text_131,cache=TRUE,eval=T,echo=T,include=T}

# number of incidents
n_marine_mammals <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Mammalia, na.rm = TRUE)) %>% pull()
n_sea_bird <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Aves, na.rm = TRUE)) %>% pull()
n_marine_turtle <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Reptiles, na.rm = TRUE)) %>% pull()
n_Teleostei <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Teleostei, na.rm = TRUE)) %>% pull()
n_Elasmobranchii <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Elasmobranchii, na.rm = TRUE)) %>% pull()
n_Holocephali <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Holocephali, na.rm = TRUE)) %>% pull()
n_Chondrostei <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Chondrostei, na.rm = TRUE)) %>% pull()
...
n_marine_mammals_species <- n_species$nbspp[n_species$classname == "Mammalia"]
n_sea_bird_species <- n_species$nbspp[n_species$classname == "Aves"]
n_marine_turtle_species <- n_species$nbspp[n_species$classname == "Reptilia"]
n_Teleostei_species <- n_species$nbspp[n_species$classname == "Teleostei"]
n_Elasmobranchii_species <- n_species$nbspp[n_species$classname == "Elasmobranchii"]
n_Holocephali_species <- n_species$nbspp[n_species$classname == "Holocephali"]
n_Chondrostei_species <- n_species$nbspp[n_species$classname == "Chondrostei"]

# Effort
total_fishing_effort <- sum(pipo$body$dataset$`Fishing Effort (das)`) %>% round(2)
total_observed_effort <- sum(pipo$body$dataset$`Total Observed Effort (das)`) %>% round(2)

text_1.3 <- glue("In total (all ecoregions combined), 

{n_marine_mammals} marine mammals ({n_marine_mammals_species} species), 
{n_sea_bird} seabirds ({n_sea_bird_species} species), 
{n_marine_turtle} marine turtles ({n_marine_turtle_species} species), 
{n_Elasmobranchii} Elasmobranchii ({n_Elasmobranchii_species} species), 
{n_Teleostei} Teleostei ({n_Teleostei_species} species), 
{n_Holocephali} Holocephali ({n_Holocephali_species} species) and
{n_Chondrostei} Chondrostei ({n_Chondrostei_species} species) 

The year {currentyear} totals {total_fishing_effort} (das) fishing effort and {total_observed_effort} (das) observed effort.
")
print(text_1.3)


# Print it
filename <- here("outputs", "descriptive_text.docx")
read_docx() |>
    body_add_par(
        text_1.3
    ) |>
...
```{r EM_descriptive_stats ,cache=TRUE,eval=T,echo=T,include=T}
NofIncidents <- D3 %>%
...
Aves <- Aves %>%
  ...
  mutate(total_incidents_per_year = sum(sum_incidents, na.rm = TRUE)) %>%
  mutate(percentage = (sum_incidents / total_incidents_per_year) * 100)
percent_aves <- round(Aves$percentage[Aves$year == currentyear & Aves$monitoringMethod == "EM"], 0)
...
Mammalia <- Mammalia %>%
  ...
  mutate(total_incidents_per_year = sum(sum_incidents, na.rm = TRUE)) %>%
  mutate(percentage = (sum_incidents / total_incidents_per_year) * 100)
percent_mammalia <- round(Mammalia$percentage[Mammalia$year == currentyear & Mammalia$monitoringMethod == "EM"], 0)

text_1.3.1 <- glue("In France, only data collected on marine mammals - mainly cetaceans - can be analyzed. EM data from other ETP 
...
# Print it
filename <- here("outputs", "descriptive_text_EM.docx")
read_docx() |>
    body_add_par(
        text_1.3.1
    ) |>
...
# Bibliography
>>>>>>> e29f10423be228450547956acc8d9abc1a18f4e7
