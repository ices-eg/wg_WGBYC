...
## days at sea
plt <- create_plot(data = daysAtSea_spatial, fill_var = "daysAtSeaF_cat", 
...
plt <- create_plot(data = daysAtSea_Ob, fill_var = "daysAtSea_Ob_cat", 
...
## monitoring coverage (%)
plt <- create_plot(data = spatialCoverage, fill_var = "Obs_Coverage_cat", 
...
D3 <- D3 %>% mutate(classname = case_when(superClass == "Reptilia" & is.na(D3$classname) ~ 'Reptilia', .default = classname))
...
plt <- ggplot(data_toPlot_noFish,aes(x = year,y = bycatch_numInd,
                                             fill = className,
                                             label = className)) +
...
plt <- ggplot(data_toPlot_elasmo, aes(x = year,y = bycatch_numInd,
                                                fill = className,
                                                label = className)) +
...
lapply(1:3, function(x){
  ...
  plt <- ggplot(data_toPlot_allPETS_list[[x]], 
                          aes(x = year,
                              y = bycatch_numInd,
                              fill = className,
                              label = className)) +
