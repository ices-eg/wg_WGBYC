...
n_fishes <- pipo$body$dataset %>% filter(variable == "Individuals") %>% summarise(sum(Teleostei, na.rm = TRUE)) %>% pull()
...
n_fish_species <- n_species$nbspp[n_species$classname == "Teleostei"]
...
text_1.3 <- glue("In total (all ecoregions combined), {n_marine_mammals} marine mammals ({n_marine_mammals_species} species), {n_sea_bird} seabirds ({n_sea_bird_species} species), {n_marine_turtle} marine turtles ({n_marine_turtle_species} species), {n_holocephali} Holocephali ({n_Holocephali_species} species) and {n_chondrostei} Chondrostei ({n_Chondrostei_species} species) were recorded as bycatch during {currentyear}. Records of {n_fish_species} fish species
from the ICES fish bycatch reference list were also reported, totaling over {n_fishes} specimens.
